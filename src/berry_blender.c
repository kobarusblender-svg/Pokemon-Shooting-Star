...
static void SetOpponentsBerryData(u16 playerBerryItemId, u8 playersNum, struct BlenderBerry *playerBerry)
{
    ...
    u16 i;
        ...
        if (playerBerryItemId == BERRY_ID_NUTPEA || playerBerryItemId == BERRY_ID_ROKA //MOD CONTEST The E-reader berries fit with the pattern of the rest, so now, they work the same,
                                                                                        ) //But for the Roka and the Nutpea berries, they choose a random set of berries.
        {
            opponentSetId = ItemIdToBerryType(RandomUniform(RNG_NONE, 1, 5));
