...
static void GetMonConditionGraphData(s16 listId, u8 loadId)
{
    ...
    if (listId != (IsConditionMenuSearchMode() ? monListPtr->listCount : monListPtr->listCount - 1))
    {
        ...
        menu->numSparkles[loadId] = GET_NUM_CONDITION_SPARKLES(GetBoxOrPartyMonData(boxId, monId, MON_DATA_SHEEN, NULL) + GetBoxOrPartyMonData(boxId, monId, MON_DATA_OVERFLOWSHEEN, NULL));
